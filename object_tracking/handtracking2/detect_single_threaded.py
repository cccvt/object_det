...
# f = detector_utils.load_pose_inference_graph()
g = detector_utils.load_inference_graph()
g_dg, g_s = g.graph, g.sess
# f = detector_utils.load_pose_inference_graph()
# f_dg, f_s = f.graph, f.sess
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # image_np_res = cv2.resize(image_np, (0, 0), fx=0.5, fy=0.5)
        im_height, im_width = image_np.shape[:2]
        # actual detection
        boxes, scores = detector_utils.detect_objects(image_np, g_dg, g_s)

        # draw bounding boxes
        detector_utils.draw_box_on_image(
            num_hands_detect, args.score_thresh, scores, boxes, im_width, im_height, image_np, None)

        # detector_utils.detect_hand_pose(image_np_res, sess_pose)
