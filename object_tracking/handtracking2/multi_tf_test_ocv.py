...
class HandModelCV():
    ...
    def set_input(self, cv2image):
        ...
        self.cvNet.setInput(cv2.dnn.blobFromImage(self.image_data, 1.0 / 127.5, (300, 300), (127.5, 127.5, 127.5), swapRB=True, crop=False))
    ...
    def detect_objects(self):
        ...
        for idx, detection in enumerate(cvOut[0, 0, :, :]):
            ...
            # (0.5397675, 0.34221676, 0.8380146, 0.86717296)


            boxes.append(box)
            scores.append(score)

            # cv2.rectangle(self.image_data, (int(left*self.im_width), int(top*self.im_height)), (int(right*self.im_width), int(bottom*self.im_height)), (23, 230, 210), thickness=2)
            # if score > self.score_thresh:
            #     print box
            #     cv2.imshow('{}'.format(idx), self.image_data)

        self.boxes = boxes
        self.scores = scores
                # cv2.rectangle(self.img, (int(left), int(top)), (int(right), int(bottom)), (23, 230, 210), thickness=2)

            # '/home/testuser/obj_det_git/object_tracking/handtracking2/hand_inference_graph/frozen_inference_graph.pb',
    ...
    def crop_images(self):
        ...
        for i in range(self.num_hands_detect):
            if self.scores[i] > self.score_thresh:
                ...
                # print box
                # (345.4512023925781, 536.329345703125, 164.26404476165771, 416.2430191040039)
                crop_img = self.image_data[int(box[2]): int(box[3]),
                                           int(box[0]): int(box[1])]
                # cv2.rectangle(self.image_data, (345, 164), (536, 416), (255, 0, 0), thickness=2)
...
if __name__ == '__main__':
    ...
    handTrak = HandModelCV(score_thresh=0.4, num_hands_detect=num_hands)
