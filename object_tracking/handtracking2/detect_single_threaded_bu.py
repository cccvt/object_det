...
if __name__ == '__main__':
    ...
    while True:
        ...
        ret, image_np_full = cap.read()
        # image_np = cv2.flip(image_np, 1)
        try:
            image_np_full = cv2.cvtColor(image_np_full, cv2.COLOR_BGR2RGB)
        except:
            print("Error converting to RGB")

	image_np_res = cv2.resize(image_np_full, (0,0), fx=0.5, fy=0.5)
	im_height, im_width = image_np_res.shape[:2]
        # actual detection
        boxes, scores = detector_utils.detect_objects(
            image_np_res, detection_graph, sess)

        # draw bounding boxes
        detector_utils.draw_box_on_image(
            num_hands_detect, args.score_thresh, scores, boxes, im_width, im_height, image_np_res)
